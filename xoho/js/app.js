...
export function buildProductCard(product) {
  ...
  const preview = product.preview_url ? storageUrl('previews', product.preview_url) : ''
  ...
  return `
    <a href="produit.html?id=${product.id}" class="product-card" aria-label="${product.name} — ${formatPrice(product.price)}">
      <div class="product-card-thumb" ${preview ? `style="background-image:url('${preview}');background-size:cover;background-position:center"` : ''}>
...
export async function loadTopSales(containerId, maxItems = 3) {
  ...
  try {
    ...
    container.innerHTML = data.length
      ? data.map(buildProductCard).join('')
